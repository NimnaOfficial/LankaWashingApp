...
import controller.MaintenanceDashboardController;
import model.User;
import javax.swing.*;
...
public class MaintenanceDashboard extends JFrame {

    private User user;
    private JPanel contentPanel;
    private MaintenanceDashboardController statsController;
    private List<SidebarButton> sidebarButtons = new ArrayList<>();

    // Theme: "Engineering Blue" (Same consistency)
    ...
    public MaintenanceDashboard(User user) {
        this.user = user;
        this.statsController = new MaintenanceDashboardController();
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        JLabel roleLabel = new JLabel("MAINTENANCE MANAGER");
        ...
        addButton(menuPanel, " Machine Status", "Machines"); // Existing Panel
        addButton(menuPanel, " Technician Mgt", "Techs");    // NEW (Need to build)
        addButton(menuPanel, " Fault Logs", "Faults");       // NEW (Need to build)
        addButton(menuPanel, " Maintenance Tasks", "Tasks"); // Existing Panel
        addButton(menuPanel, " Reports", "Reports");
    ...
    private void handleMenuAction(String command) {
        ...
        switch (command) {
            ...
            case "Machines":
                contentPanel.add(new MachinePanel(), BorderLayout.CENTER);
                break;
            case "Techs":

                contentPanel.add(new TechnicianPanel(), BorderLayout.CENTER);
                break;
            case "Faults":
                contentPanel.add(new FaultLogPanel(), BorderLayout.CENTER);
                break;
            case "Tasks":
                contentPanel.add(new TaskPanel(), BorderLayout.CENTER);
                break;
            case "Reports":
                contentPanel.add(new MaintenanceReportPanel(), BorderLayout.CENTER);
                break;
        }
    ...
    private void loadHomeView() {
        ...
        // 1. Machines needing attention
        gbc.gridx = 0; gbc.gridy = 0; gbc.insets = new Insets(10, 20, 10, 20);
        contentPanel.add(createStatsCard(
                "Machines Down",
                statsController.getCriticalMachineCount(),
                new Color(220, 53, 69) // Red for Alert
        ), gbc);

        // 2. Available Technicians
        gbc.gridx = 1;
        contentPanel.add(createStatsCard(
                "Techs Available",
                statsController.getAvailableTechnicianCount(),
                new Color(40, 167, 69) // Green for Available
        ), gbc);
        ...
        contentPanel.add(createStatsCard(
                "Monthly Tasks",
                statsController.getPendingTaskCount(),
                ACCENT_COLOR // Blue
        ), gbc);
    }
    ...
    private JPanel createStatsCard(String title, String value, Color color) {
        ...
        t.setBorder(new EmptyBorder(10, 20, 0, 0));
        v.setBorder(new EmptyBorder(0, 20, 10, 0));
    ...
    // --- INNER CLASSES (Same as Production Dashboard) ---
